...
RowScope* UpdateBean::curRow() { return rowStack.top().get_object(); }
...
void UpdateBean::commitRow(shared_connection con, sqlid_t rowid)
{
  ...
  if (rowid != curRow()->id) throw std::runtime_error("rowid mismatch");
...

  {
    ...
    if (rc != SQLITE_DONE) database_error::database_assert(rc, con);
  }

  if (!rowStack.size()) throw std::logic_error("UpdateVisitor: commit row, but no row started");
