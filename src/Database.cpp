...
Database::Database(std::string fname) : mx(NULL) { open(fname); }
...
void Database::open(std::string fname)
{
  ...
  try {
    ...
    if (rc != SQLITE_OK) throw database_error(std::string("database error: ") + sqlite3_errmsg(db));
...
{
  if (!mx) throw std::logic_error("register bean classes first");
...
sqlid_t Database::allocId(std::string table) { return allocId(con, table); }

void Database::dbExecQuery(std::string query) { return dbExecQuery(con, query); }

std::vector<sqlid_t> Database::dbSelectIds(shared_connection con, const std::string table,
                                           const std::string condition, const std::string orderBy)
{
  std::string where;
  if (condition.size()) where = " WHERE " + condition;
  std::string order;
  if (orderBy.size()) order = " ORDER BY " + orderBy;
  std::string query = "SELECT " + std::string(HIBERLITE_PRIMARY_KEY_COLUMN) + " FROM " + table +
                      where + order + ";";
  SQLiteSelect sel(con, query);
  std::vector<sqlid_t> ans;
  while (sel.step()) ans.push_back(sel.get_int64(0));
  return ans;
}

std::vector<sqlid_t> Database::dbSelectChildIds(shared_connection con, std::string table,
                                                sqlid_t parent)
{
  return dbSelectIds(
      con, table, std::string(HIBERLITE_PARENTID_COLUMN) + "=" + Transformer::toSQLiteValue(parent),
