...
class X {
 ...
 public:
  ...
  bool operator==(const X& x) { return seti == x.seti && vvi == x.vvi && mm == x.mm; }
};
...
TEST_CASE()
{
  ...
  for (unsigned int i = 0; i < 5; i++) {
    ...
    for (unsigned int i = 0; i < 5; i++) {
      ...
      if (!ptr.get_object()) CAPTURE(x.name);
    ...
    if (a.get_id() % 10 == 0) CAPTURE(std::string("save obj with name ") + (*a).s);
    CHECK((a.get_id() % 10) != 0);
  }
}
...
TEST_CASE()
{
  ...
  if (!a2.get_object()) CAPTURE(std::string("load obj with name ") + (*a2).s);
  CHECK(a2.get_object());
}
...
TEST_CASE()
{
  ...
  for (unsigned int i = 0; i < 100; i++) {
    ...
    vector<string> vs;
    for (unsigned int i = 0; i < 5; i++) vs.push_back(randstr[(17 + 7 * i) % NS]);
    for (unsigned int i = 0; i < 10; i++) x.mm[vs][randstr[(19 * i) % NS]] = randstr[(23 * i) % NS];
  }
...
namespace hiberlite {
template <class A>
void hibernate(A& ar, Gender& value, const unsigned int)
{
  ar& db_atom<Gender>(value);
}

template <>
inline std::string db_atom<Gender>::sqliteStorageClass()
{
  return "INTEGER";
}
...
void db_atom<Gender>::loadValue(Stmt& res, Arg& arg)
{
  val = static_cast<Gender>(res.get_int(arg));
}

template <>
inline void db_atom<Gender>::bindValue(sqlite3_stmt* stmt, int col)
{
