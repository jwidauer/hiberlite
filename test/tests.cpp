...
class WrappedBytes {
 ...
 public:
  ...
  bool operator==(const WrappedBytes& wrappedBytes) { return bytes == wrappedBytes.bytes; }
};
...
struct Tester {
  void test1()
  {
    ...
    for (size_t ci = 0; ci < msg.size(); ci++) cout << "model check reported: " << msg[ci] << endl;
    ...
    for (unsigned int i = 0; i < 5; i++) {
      ...
      if (a.get_id() % 10 == 0) cout << "save obj with name " << (*a).s << endl;
    }
  }
  void test2()
  {
  ...
  void test3()
  {
...

    {
      ...
      if (!(*xptr == x)) throw std::runtime_error("load failed");
    }
  }
  ...
  void test4()
  {
...

    {
      ...
      if (!(*wb_ptr == wrappedBytes)) throw std::runtime_error("BLOB load failed");
    }
  }
};
