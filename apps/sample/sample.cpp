...
void createDB()
{
  ...
  const char* names[5] = {"Stanley Marsh", "Kyle Broflovski", "Eric Theodore Cartman",
                          "Kenneth McCormick", "Leopold Stotch"};
...
void printDB()
{
  ...
  for (size_t j = 0; j < v.size(); j++) {
    ...
    cout << "age=" << v[j]->age << "\t";
    cout << "bio={";
    for (size_t i = 0; i < v[j]->bio.size(); i++) i &&cout << ", ", cout << v[j]->bio[i];
    cout << "}]\n";
  }
}
