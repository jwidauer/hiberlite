...
class shared_cnt_obj_pair : noncopyable {
 ...
 public:
  inline C* getRes() { return res; }
  inline virtual ~shared_cnt_obj_pair()
  {
    if (refCount) throw std::logic_error("resource is busy");
    delete res;
  }
  inline int getRefCount() { return refCount; }
  inline void inc_pair_users() { refCount++; }
  inline void dec_pair_users() { refCount--; }
};
...
class shared_res {
 protected:
  ...
  inline void freeRes()
  {
    if (!res) return;
    res->dec_pair_users();
    if (res->getRefCount() == 0) delete res;
  }
 ...
 public:
  ...
  {
    if (!tg) throw std::logic_error("no reason to manage NULL object");
  ...
  inline shared_res(const shared_res<C>& x) : res(NULL) { takeRes(x.res); }

  inline virtual ~shared_res() { freeRes(); }
  ...
  inline C* operator->() { return get_object(); }
  ...
  inline bool operator==(const shared_res<C>& x) const { return x.res == res; }
  inline bool operator<(const shared_res<C>& x) const { return res < x.res; }
  inline operator bool() const { return res != NULL; }
};
...
class bean_key {
 public:
  ...
  bool operator<(const bean_key& k) const { return (id < k.id) || ((id == k.id) && (con < k.con)); }
};
