...
{
  collection_nvp<E, stl_stream_adapter<E, std::vector<E>>> body(
      "items", stl_stream_adapter<E, std::vector<E>>(v));
  ar& body;
}
...
{
  typedef Key ElType;
  collection_nvp<ElType, stl_stream_adapter<ElType, std::set<Key>>> body(
      "items", stl_stream_adapter<ElType, std::set<Key>>(m));
  ar& body;
}
...
{
  typedef Key ElType;
  collection_nvp<ElType, stl_stream_adapter<ElType, std::list<Key>>> body(
      "items", stl_stream_adapter<ElType, std::list<Key>>(m));
  ar& body;
}
...
void hibernate(A& ar, std::map<Key, Data>& m, const unsigned int)
{
  ...
  collection_nvp<ElType, stl_map_adapter<Key, Data>> body("items", stl_map_adapter<Key, Data>(m));
  ar& body;
}

#define HIBERLITE_DEF_DB_ATOM(T, GetFromDB, PutToDB, StorClass)  \
  template <class A>                                             \
  void hibernate(A& ar, T& value, const unsigned int)            \
  {                                                              \
    ar& db_atom<T>(value);                                       \
  }                                                              \
  template <>                                                    \
  inline std::string db_atom<T>::sqliteStorageClass()            \
  {                                                              \
    return StorClass;                                            \
  }                                                              \
  template <>                                                    \
  template <class Stmt, class Arg>                               \
  void db_atom<T>::loadValue(Stmt& res, Arg& arg)                \
  {                                                              \
    val = res.GetFromDB(arg);                                    \
  }                                                              \
  template <>                                                    \
  inline void db_atom<T>::bindValue(sqlite3_stmt* stmt, int col) \
  {                                                              \
