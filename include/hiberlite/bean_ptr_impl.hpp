...
C* real_bean<C>::operator->()
{
  ...
  loadLazy();
  if (!obj) throw std::runtime_error("NULL pointer exception!");
  return obj;
}
...
{
  if (!obj && key.id != Database::NULL_ID) obj = Database::dbLoad<C>(key);
}
...
void bean_ptr<C>::hibernate(Archive& ar)
{
  ...
  if (ar.is_loading()) *this = Registry<C>::get(bean_key(ar.getConnection(), tmp_id));
}
...
{
  if (!shared_res<real_bean<C>>::get_object()) return Database::NULL_ID;
