...
#define HIBERLITE_STRINGIFY_NUM(VALTYPE)               \
  ...
  {                                                    \
    ...
    *(p--) = 0;                                        \
    VALTYPE tmp = val;                                 \
    bool needmin = 0;                                  \
    if (tmp < (VALTYPE)0) needmin = 1, tmp *= -1;      \
    do {                                               \
      *(p--) = '0' + (tmp % 10);                       \
      tmp /= 10;                                       \
    } while (tmp);                                     \
...
#undef HIBERLITE_STRINGIFY_NUM
  ...
  static std::string toSQLiteValue(const std::string val) { return toSQLiteValue(val.c_str()); }

  static std::string toSQLiteValue(const bool val) { return val ? "1" : "0"; }
};
